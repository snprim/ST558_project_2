...
We again use `caret` package and set the method as `gbm`. We use repeated cross validation (`repeatedcv`) and set the `tuneLength` as 10 and let the model chooses the best model automatically. Then we use the model to predict `cnt` on the test data. Finally, we calculate RMSE to check the fit of the model.  
...
From the output, we can conclude that `r model(treeResult, boostedResult)` is the better model for `r params$weekday` data, because it has better performance in terms of RMSE, Rsquared, and MAE.  
